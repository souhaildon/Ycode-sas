...
struct Reservation rdv_tab[500];//rdv_tab:tableau
int cpt = 0; // cpt:compteur
int id = 1;

const char* rdv_statut[] = {"valide", "reporte", "annule", "traite"};//rdv_statut:tableau des statuts de reservation
const int nombre_statuts = sizeof(rdv_statut) / sizeof(rdv_statut[0]);

int ver_date(const char *date) {
    int an, mois, jour;
    if (sscanf(date, "%d-%d-%d", &jour, &mois, &an) != 3) return 0;
...
int ver_n() { //ver_n:pour gerer les entrees
    int var;
    while (1) {
        if (scanf("%d", &var) != 1 || var < 0) {
            printf("Entrer invalide. Veuillez entrer un nombre positif : ");
            while (getchar() != '\n');
        } else {
            while (getchar() != '\n');
            return var;
...
void rechercher_par_ref(int id, struct Reservation* reserve) {
    for (int i = 0; i < cpt; i++) {
        if (rdv_tab[i].id == id) {
            *reserve = rdv_tab[i];
            return; // Réservation trouvé
        }
    }
    reserve->id = -1; // Indique que la réservation n'a pas été trouvé
}
...
void modifier_reservation(int id) {
    ...
    if (rendvous.id != -1) {
        printf("Modifier la reservation (ID: %d)\n", rendvous.id);
        printf("Nouveau nom (actuel: %s): ", rendvous.nom);
        scanf(" %[^\n]s", rendvous.nom);

        printf("Nouveau prenom (actuel: %s): ", rendvous.prenom);
        scanf(" %[^\n]s", rendvous.prenom);
        ...
        printf("Nouveau age (actuel: %d): ", rendvous.age);
        rendvous.age = ver_n();
        ...
        int choix_statut;
        do {
            printf("Entrez le numero du statut choisi: ");
            choix_statut = ver_n();
        ...
        while (1) {
            ...
            if (ver_date(new_date)) {
                ...
                printf("Date invalide. Veuillez entrer une date valide (jj-m-an): ");
            }
        }
...
void afficher_statistiques(int choix) {
    ...
    //memset ;fonction permet de remplir une zone mémoire, identifiée par son adresse et sa taille, avec une valeur précise.
...
void insertion_defaut() {
    ajouter_reserv("Lasfar", "Souhail", "0708878867", 24, "valide", "20-10-2024");
    ajouter_reserv("Abdi", "AB", "0612345678", 30, "annule", "10-10-2024");
    ajouter_reserv("Hamdi", "Salma", "0623456789", 40, "traitee", "11-11-2024");
    ajouter_reserv("Elbarry", "Anouar", "0667890123", 50, "reportee", "04-10-2024");
    ajouter_reserv("Labit", "Ayoub", "0734567890", 15, "valide", "05-11-2024");
    ajouter_reserv("Allala", "Belal", "0611223344", 20, "annulee", "12-12-2024");
    ajouter_reserv("Maizou", "Nabil", "0611443322", 22, "traitee", "02-01-2025");
    ajouter_reserv("Pianiste", "Amine", "0712113344", 35, "reportee", "07-11-2024");
    ajouter_reserv("Hamidden", "Moad", "0612340987", 28, "valide", "09-12-2024");
    ajouter_reserv("Brahimi", "Hassan", "0680907060", 33, "annulee", "03-01-2025");
}
...
int main() {
    ...
    do {
        ...
        switch (choix) {
            ...
            case 3: {
                int id;
                printf("Entrer la reference (ID) a supprimer: ");
                id = ver_n();
            ...
            case 5: {
                int id;
                printf("Entrer la reference unique (ID) pour consulter les details: ");
                id = ver_n();
            ...
            case 6: {
                printf("Choisissez le type de statistique a afficher:\n");
                printf("1. Moyenne d\'age\n");
                printf("2. Nombre de patients par tranche d\'age\n");
            ...
            case 7: {
                ...
                switch (tri) {
                    case 1:
                        ...
                        printf("Reservations tri par nom.\n");
                        break;
                    case 2:
                        tri_reservation_par_statut(ord);
                        printf("Reservations tri par statut.\n");
                        break;
                    case 3:
                        tri_reservation_par_date(ord);
                        printf("Reservations tri par date.\n");
                        break;
                    default:
                        printf("Tri invalide.\n");
                }
